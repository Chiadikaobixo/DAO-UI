...
import {
  ...
  CXONFT_COLLECTION_CONTRACT_ABI,
...
export default function Home() {
  ...
  const [fakeNftPurpose, setFakeNftPurpose] = useState("");
  ...
  const createProposal = async () => {
    try {
      ...
      const txn = await daoContract._createProposal(fakeNftTokenId, fakeNftPurpose);
      setLoading(true);
      await txn.wait();
  ...
  const voteOnProposal = async (proposalId, _vote) => {
    try {
      ...
      const txn = await daoContract._voteOnProposal(proposalId, vote);
      setLoading(true);
      await txn.wait();
      setLoading(false);
  ...
  const executeProposal = async (proposalId) => {
    try {
      const signer = await getProviderOrSigner(true);
      const daoContract = getDaoContractInstance(signer);
      const txn = await daoContract._executeProposal(proposalId);
      setLoading(true);
      await txn.wait();
      setLoading(false);
      await fetchAllProposals();
      ...
      return (
        <div className={styles.container}>
          ...
          <input
            placeholder="purpose"
            type="text"
            onChange={(e) => setFakeNftPurpose(e.target.value)}
          />
